...
class RockPaperScissorsEngine(SimulationEngine):
    ...
    def make_moves(self, moves: List[str]) -> None:
        ...
        move1 = next(move for move in moves if move.startswith(">p1 ")).split(" ")[1]
        move2 = next(move for move in moves if move.startswith(">p2 ")).split(" ")[1]
...
class TestMonteCarloRunner(unittest.TestCase):
    ...
    def test_monte_carlo_rock_paper_scissors(self):
        ...
        self.assertLess(rock_result.wins, paper_result.wins)
        self.assertLess(paper_result.wins, scissors_result.wins)
